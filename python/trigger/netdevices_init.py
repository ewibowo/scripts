...
class NetDevice(object):
    ...
    def _cleanup_attributes(self):
        ...
        # Lowercase the deviceTags for completeness.
        if self.deviceTags is not None:
            self.deviceTags = self.deviceTags.lower()
    ...
    def _set_requires_async_pty(self):
        ...
        """
        RULES = (
            self.vendor in ('a10', 'arista', 'aruba', 'cisco', 'riverbed', 'force10'),
            self.is_brocade_vdx(),
        )
        return any(RULES)
    ...
    def dump(self):
        ...
        print '\tHost IPv4:        ', dev.ipv4
        print '\tHost Port:        ', dev.nodePort
        print '\tHost Protocol:    ', dev.nodeProtocol
        ...
        print '\tOperating System: ', dev.operatingSystem
        print '\tSite:             ', dev.site
        print '\tLocation:         ', dev.location
        print '\tRoom:             ', dev.room
        print '\tCoordinates:      ', dev.coordinate
        print
        print '\tDevice Tags:      ', dev.deviceTags
...
class NetDevices(DictMixin):
    ...
    class _actual(object):
        ...
        def get_devices_by_tag(self, devtag):
            """
            Returns a list of NetDevice objects with matching deviceTags.

            This gives the user more granular "one-off" customized runs.

            Examples of deviceTags: ['projectA', 'projectB', 'test-run1']
            """
            return [x for x in self._dict.values() if x.deviceTags == devtag]
